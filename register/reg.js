...
angular.module("invente",['ngAnimate'])
.controller("regFormController", ['$scope', '$http', '$timeout', function($scope, $http, $timeout){
            ...
            $http.post("onlinereg.php",$scope.data, {}).then(
                function(res){
                    ...
                    if(res.data.success === true) {
                        $scope.recommended = true;
                        $scope.errMsg = "Successfully Registered";
                        $timeout(function(){$scope.popup = false;}, 6000);
                    }
                    else {
